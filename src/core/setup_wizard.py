...
# FIX: Removed the unnecessary 'litellm' import.
...
def get_available_models(api_key: str, base_url: Optional[str], models_endpoint: Optional[str]) -> List[str]:
    """Fetches the list of available models from a provider's API endpoint directly."""
...
def configure_engine() -> Optional[Dict[str, Any]]:
    ...
    while True:
        ...
        if not all(key in provider_config for key in required_keys):
            console.print(f"[bold red]Error:[/bold red] The configuration for '{provider_display_name}' in `providers.json` is incomplete.")
            continue
        ...
        with console.status("[yellow]Fetching available models...", spinner="dots"):
            available_models = get_available_models(api_key, provider_config.get("base_url"), provider_config.get("models_endpoint"))
